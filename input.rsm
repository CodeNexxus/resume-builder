name: John
surname: Doe
job_title: Software Engineer
phone: +989303016386
city: New York
gmail: johndoe@gmail.com
birth: 2000-10-11

summary: Experienced software engineer with a passion for developing innovative programs that expedite the efficiency and effectiveness of organizational success. Well-versed in technology and writing code to create systems that are reliable and user-friendly.

hard_skills:
    Java, 4
    Python, 4
    JavaScript, 4
    SQL, 3
    Git, 5

soft_skills:
    Problem Solving
    Team Collaboration
    Time Management

languages:
    English
    Spanish

certificates:
    name: Certified Java Developer
    institution: Oracle
    link: https://www.oracle.com/java/certification/

    name: AWS Certified Solutions Architect
    institution: Amazon
    link: https://aws.amazon.com/certification/certified-solutions-architect-associate/

socials:
    linkedin: https://linkedin.com/in/johndoe
    github: https://github.com/johndoe

projects:
    title: Project Management System
    description: Developed a web-based project management system using Java and Spring Boot
    link: https://github.com/johndoe/project-management-system

    title: E-commerce Platform
    description: Built an e-commerce platform using Python and Django
    link: https://github.com/johndoe/e-commerce-platform

work_experience:
    company: TechCorp
    position: Senior Software Engineer
    start_date: 2018-06-01
    end_date: Present
    responsibilities:
        Lead a team of software engineers to develop scalable software solutions
        Implemented new features and maintained existing codebase for various projects
        Collaborated with cross-functional teams to deliver high-quality products

    company: Web Solutions
    position: Software Engineer
    start_date: 2015-01-01
    end_date: 2018-05-31
    responsibilities:
        Developed and maintained web applications using JavaScript and Node.js
        Improved application performance and fixed bugs
        Assisted in the design and architecture of new projects

educations:
    institution: State University
    degree: Bachelor of Science in Computer Science
    start_date: 2010-09-01
    end_date: 2014-06-30

    institution: Tech Institute
    degree: Master of Science in Software Engineering
    start_date: 2015-09-01
    end_date: 2017-06-30